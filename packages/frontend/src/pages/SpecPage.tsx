      ...
      {!loadingHistory && displayGuide && (
        <>
          ...
          {displayGuide.changelog && displayGuide.changelog.items?.length > 0 && (
            <div className="mb-2 border border-gray-200 dark:border-gray-800 rounded-lg bg-gray-50 dark:bg-gray-900">
              <button
                onClick={() => setShowChangelog(!showChangelog)}
                className="w-full flex items-center gap-2 px-4 py-2.5 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors text-left"
              >
                <svg
                  className={`w-3.5 h-3.5 text-gray-400 transition-transform shrink-0 ${showChangelog ? 'rotate-90' : ''}`}
                  viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"
                >
                  <polyline points="9 6 15 12 9 18" />
                </svg>
                <svg className="w-3.5 h-3.5 text-gray-400 dark:text-gray-500 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
